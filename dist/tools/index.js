...
export function registerTools(server) {
    ...
    server.registerTool('analyzeProduct', {
        ...
        if (!nameOrBarcode?.trim()) {
            return { content: [{ type: 'text', text: 'Provide a product name or barcode.' }], isError: true };
        }
        const productData = await findProduct(nameOrBarcode);
        if (!productData?.product) {
            return { content: [{ type: 'text', text: `"${nameOrBarcode}" not found. Try searchProducts first.` }], isError: true };
        }
        ...
        try {
            ...
            return { content: [{ type: 'text', text: header + getResponseText(res) }] };
        }
        catch {
            return { content: [{ type: 'text', text: `# ${p.product_name}\nBrand: ${p.brands}\nNutri-Score: ${p.nutriscore_grade}\nIngredients: ${p.ingredients_text}` }] };
        }
    });
    /**
    ...
    server.registerTool('compareProducts', {
        ...
        if (!nameOrBarcode1?.trim() || !nameOrBarcode2?.trim()) {
            return { content: [{ type: 'text', text: 'Provide both products.' }], isError: true };
        }
        const [d1, d2] = await Promise.all([findProduct(nameOrBarcode1), findProduct(nameOrBarcode2)]);
        if (!d1?.product)
            return { content: [{ type: 'text', text: `"${nameOrBarcode1}" not found.` }], isError: true };
        if (!d2?.product)
            return { content: [{ type: 'text', text: `"${nameOrBarcode2}" not found.` }], isError: true };
        try {
            const res = await requestSampling(server, {
            ...
            return { content: [{ type: 'text', text: `# ${d1.product.product_name} vs ${d2.product.product_name}\n\n${getResponseText(res)}` }] };
        }
        catch {
    ...
    server.registerTool('suggestRecipes', {
        ...
        if (!nameOrBarcode?.trim()) {
            return { content: [{ type: 'text', text: 'Provide a product.' }], isError: true };
        }
        const productData = await findProduct(nameOrBarcode);
        if (!productData?.product) {
            return { content: [{ type: 'text', text: `"${nameOrBarcode}" not found.` }], isError: true };
        }
        try {
            const res = await requestSampling(server, createRecipeSuggestionRequest(productData.product));
            return { content: [{ type: 'text', text: `# Recipes using ${productData.product.product_name}\n\n${getResponseText(res)}` }] };
        }
        catch (e) {
            return { content: [{ type: 'text', text: `Recipe generation failed: ${e.message}` }], isError: true };
        }
    });
